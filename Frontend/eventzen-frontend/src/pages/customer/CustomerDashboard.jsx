...
import { getAllEvents, getEventsByCategory, getEventsByStatus, createBooking } from "../../api/eventApi";
...
const CustomerDashboard = () => {
    ...
    const handleBookEvent = async (eventId) => {
        ...
        try {
            await createBooking({ eventId, budget: budgets[eventId] ? parseFloat(budgets[eventId]) : null }, token);
            setBookingSuccess(eventId);
        } catch (err) {
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-gray-500 mt-1">
                                Browse and book events for your occasion
                            </p>
                        </div>
                        <Link
                            to="/my-bookings"
                                    ...
                                    {bookingSuccess === event.id ? (
                                        ...
                                        <div className="space-y-2">
                                            <input
                                                ...
                                                value={budgets[event.id] || ""}
                                                onChange={(e) => setBudgets(prev => ({ ...prev, [event.id]: e.target.value }))}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                            />
                                            <button
                                                onClick={() => handleBookEvent(event.id)}
                                                disabled={bookingLoading === event.id || event.status === "CANCELLED" || event.status === "COMPLETED"}
                                                ...
                                                {bookingLoading === event.id ? "Booking..." : "Book This Event"}
                                            </button>
                                        </div>
                                    )}
                                </div>
